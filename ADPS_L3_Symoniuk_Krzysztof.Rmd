...
```{r}
# H₀: dane ~ N(μ̂, σ̂)
# H₁: dane ≠ rozkład normalny
x = df_KGH_1rok$Najnizszy_zm
mean(x)
sd(x)
length(x)
#Test Kołmogorowa-Smirnowa (parametry z próby)
...
library(moments)
jarque.test(x)
#Test skośności i kurtozy
agostino.test(x)
anscombe.test(x)

```

Przy poziomie istotności α = 0.05 test Shapiro-Wilka oraz test Jarque-Bera dały p-wartości mniejsze od 0.05, co prowadzi do odrzucenia hipotezy zerowej o normalności rozkładu procentowych zmian najniższych cen akcji KGHM w ciągu ostatniego roku. Oznacza to, że rozkład badanych stóp zwrotu istotnie odbiega od rozkładu normalnego.
...
Brak podstaw do odrzucenia hipotezy zerowej o równości średnich czasów pracy łożysk wykonanych z dwóch różnych materiałów.
...
Brak podstaw do odrzucenia hipotezy zerowej o braku różnic między czasami pracy łożysk wykonanych z dwóch różnych materiałów.

```{r}
#Obliczenie prawdopodobieństwa
test = wilcox.test(x, y, exact = TRUE)
 
P_hat = as.numeric(test$statistic) / (length(x) * length(y))
P_hat

```
Na podstawie statystyki testu Manna–Whitneya 
W=75
W=75 oszacowano prawdopodobieństwo, że losowo wybrane łożysko wykonane z pierwszego materiału będzie pracowało dłużej niż łożysko wykonane z drugiego materiału. Otrzymano wartość 
P^(X>Y)=0.75
P
^
(X>Y)=0.75. Oznacza to, że w około 75% przypadków czas pracy łożyska z materiału I jest dłuższy niż czas pracy łożyska z materiału II.
***
...
H0: wynik meczu gospodarzy jest niezależny od kraju (ligi)
H1: wynik meczu gospodarzy zalez˙y od kraju (ligi)
...
x_ANG = c(193, 96, 91)
x_ESP = c(194, 95, 91)
xx = cbind(x_DE, x_PL, x_ANG, x_ESP)
chisq.test(xx)
```
Wartość p = 0,1116 jest większa od poziomu istotności alfa = 0,05, brak zależności między krajem, w którym rozgrywana jest liga, a wynikiem meczu gospodarzy
***